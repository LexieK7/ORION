...
class TransMIL(BaseAggregator):
    def __init__(self, num_classes, input_dim=512, pos_enc='PPEG', **kwargs):
        super(BaseAggregator, self).__init__()
        self.pos_layer = PPEG(dim=256)
        self.pos_enc = pos_enc
        print(f'Using {self.pos_enc} positional encoding')
        self._fc1 = nn.Sequential(nn.Linear(input_dim, 256), nn.ReLU())
        self.cls_token = nn.Parameter(torch.randn(1, 1, 256))
        self.num_classes = num_classes
        self.layer1 = NystromTransformerLayer(dim=256)
        self.layer2 = NystromTransformerLayer(dim=256)
        self.norm = nn.LayerNorm(256)
        self._fc2 = nn.Linear(256, self.num_classes)
    ...
    def forward(self, x, coords=None):
        ...
        return logits#, h # h is used to predict alpha
